...
create index IX_52548162 on ActivitySubscription (entityId, receiverId);
create index IX_7A05AC5F on ActivitySubscription (entityId, receiverId, activitytype);
...
create index IX_E8E8D4AB on ContestPhase (ContestPK, phaseActiveOverride);

create index IX_1652B7B9 on ContestPhaseColumn (ContestPhasePK);

create index IX_18128B3C on ContestPosition (ContestPK);

create index IX_C12DBC2E on ContestTeamMember (contestId);
...
create index IX_E0E8AD9D on Debate (debateId, status);
create index IX_6D6150B3 on Debate (debateId, treeVersion);
create index IX_63B3DE31 on Debate (debateId, version);

create index IX_95A9F7D1 on DebateCategoryMap (debateCategoryPK, debateId);

create index IX_3E3F7767 on DebateComment (debateItemId);

create index IX_3BF17C18 on DebateDiscussionMap (debateMessageId);

create index IX_4A5B9138 on DebateItem (debateItemId, itemVersion);
create index IX_31DF2903 on DebateItem (debateItemId, status);
create index IX_FD2326BE on DebateItem (debateItemId, treeVersion, updated);
create index IX_318CCF8B on DebateItem (debateItemId, version);

create index IX_E7EDD573 on DebateItemReference (debateId, status);
create index IX_DF5691DB on DebateItemReference (debateItemId, itemVersion, status);
create index IX_8BC5A026 on DebateItemReference (debateItemId, status);

create index IX_94A04827 on DebateItemVote (debateItemId);
create index IX_F4028961 on DebateItemVote (debateItemId, userId);
create index IX_9A3DA74E on DebateItemVote (userId);

create index IX_4D6CD1CA on DebateItemVoteStats (debateItemId);
...
create index IX_1FE5354E on HelpUserSetting (userId, messageId);
...
create index IX_B5DA7D86 on Message (fromId);

create index IX_9A95C27A on MessageRecipient (messageId);
create index IX_47B3DD72 on MessageRecipient (userId);

create index IX_EEEE8128 on MessageRecipientStatus (messageId);
create index IX_F14C3162 on MessageRecipientStatus (messageId, userId);
create index IX_F7AD4D84 on MessageRecipientStatus (userId);
create index IX_EFD81ADA on MessageRecipientStatus (userId, archived);

create index IX_C3FB85E on MessagingIgnoredRecipients (email);
create index IX_3E72AD68 on MessagingIgnoredRecipients (userId);

create index IX_B76E67F2 on MessagingMessageConversion (messageId, conversionTypeId);

create index IX_7973D4D8 on MessagingMessageConversionType (name);

create index IX_5DCD169 on MessagingUserPreferences (userId);

create index IX_87D6D305 on MigrationMapping (newId);
create index IX_C7A33C0C on MigrationMapping (oldId);
...
create index IX_8F90A3DC on ModelInputGroup (parentGroupPK);
...
create index IX_99B1604E on UserHelpSetting (userId, messageId);
