...
<%List<MBMessage> positions = (List<MBMessage>) portletSession.getAttribute(ModelConstants.MODEL_POSITIONS);
List<MBMessage> questions = (List<MBMessage>) portletSession.getAttribute(PlanConstants.QUESTIONS);
Map<MBMessage, List<MBMessage>> questionPositions = 
       (Map<MBMessage, List<MBMessage>>) portletSession.getAttribute(PlanConstants.QUESTION_POSITIONS);
String modelId=ParamUtil.getString(request, ModelConstants.MODEL_ID);
%>
<%
...
<script type="text/javascript">
	...
	/**
	 * Updates positions for a model with selected values.
	 */
	function <portlet:namespace />updatePositions() {
		...
		document.<portlet:namespace />fm.<portlet:namespace /><%= ModelConstants.POSITIONS %>.value = positionsString;
	 ...
	 *
	 * If window shouldn't be closed, it get's initialized with model positions.
	 */ 
...
<c:set var="questionPositions" value="<%=questionPositions%>" />
