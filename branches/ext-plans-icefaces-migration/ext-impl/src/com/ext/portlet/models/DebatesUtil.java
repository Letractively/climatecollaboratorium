...
import com.ext.portlet.plans.service.persistence.PlansUserSettingsPersistenceImpl;
...
public class DebatesUtil {
	 ...
	 */
	public static List<MBMessageFlag> getMessageFlags(long messageId)
			throws SystemException {

		// create custom query and execute it
		DynamicQuery dynamicQuery = DynamicQueryFactoryUtil
				.forClass(MBMessageFlag.class);
		Criterion criterionMessageId = RestrictionsFactoryUtil.eq(
				DebatesConstants.MESSAGE_ID, messageId);
		dynamicQuery.add(criterionMessageId);

		List<Object> tmpFlags = MBMessageFlagLocalServiceUtil
	 ...
	 */
	public static int getMessageType(long messageId) throws SystemException,
		...
		throw new MessageTypeException("Message has no type associated.");
	}
	 ...
	 *             if message has no type associated
	 */
	public static String getMessageTypeName(long messageId)
			throws SystemException, MessageTypeException {
		int type = getMessageType(messageId);

		Map<Integer, String> typeNamesMap = new HashMap<Integer, String>();
		typeNamesMap.put(DebatesConstants.ISSUE_MSG_TYPE,
				DebatesConstants.ISSUE_MSG_TYPE_NAME);
		typeNamesMap.put(DebatesConstants.POSITION_MSG_TYPE,
				DebatesConstants.POSITION_MSG_TYPE_NAME);
		typeNamesMap.put(DebatesConstants.ARGUMENT_CON_MSG_TYPE,
				DebatesConstants.ARGUMENT_CON_MSG_TYPE_NAME);
		typeNamesMap.put(DebatesConstants.ARGUMENT_PRO_MSG_TYPE,
				DebatesConstants.ARGUMENT_PRO_MSG_TYPE_NAME);
	 ...
	 */
	public static void setMessageType(long messageId, int messageType)
		...
		int types[] = { DebatesConstants.ISSUE_MSG_TYPE,
				DebatesConstants.POSITION_MSG_TYPE,
				DebatesConstants.ARGUMENT_PRO_MSG_TYPE,
				DebatesConstants.ARGUMENT_CON_MSG_TYPE };
		...
		if (flag == null) {
			long messageFlagId = CounterLocalServiceUtil
					.increment(MBMessageFlag.class.getName());
			flag = MBMessageFlagLocalServiceUtil
					.createMBMessageFlag(messageFlagId);
	 ...
	 */
	public static MBCategory getMainCategory(PortletRequest request)
			throws Exception {
		PortletPreferences preferences = request.getPreferences();
		long categoryId = Long.parseLong(preferences.getValue(
				DebatesConstants.CATEGORY_ID, "-1"));
		...
		if (categoryId <= 0) {
			...
			List<MBCategory> categories = MBCategoryLocalServiceUtil
					.getCategories(groupId, 0);

			for (MBCategory category : categories) {
				if (category.getName().equals(
						DebatesConstants.DEFAULT_DEBATES_CATEGORY_NAME)) {
					categoryId = category.getCategoryId();
				}
			}
			if (categoryId <= 0) {
				result = createMBCategory(request, 0,
						DebatesConstants.DEFAULT_DEBATES_CATEGORY_NAME);
		...
		preferences.setValue(DebatesConstants.CATEGORY_ID, Long
				.toString(categoryId));
	 ...
	 */
	public static MBCategory getSubcategory(MBCategory category, String type)
			throws SystemException, NoSuchCategoryException {

		List<MBCategory> categories = null;
		if (category.getCategoryId() == 0) {
			categories = MBCategoryLocalServiceUtil.getCategories(category
					.getGroupId());
		} else {
			categories = MBCategoryLocalServiceUtil.getCategories(category
					.getGroupId(), category.getCategoryId());
		}
		...
		throw new NoSuchCategoryException("Can't find category named \"" + type
				+ "\" within category: " + category.getCategoryId());
	}

	public static MBCategory searchCategory(MBCategory category, String type)
			throws SystemException, NoSuchCategoryException {
		if (category.getName().equals(type)) {
			...
			if (category.getCategoryId() == 0) {
				try {
					categories = MBCategoryLocalServiceUtil
							.getCategories(category.getGroupId());
				} catch (Exception e) {
					return null;
				}
			} else {
			...
			for (MBCategory tmpCategory : categories) {
				category = searchCategory(tmpCategory, type);
			}
			if (category != null) {
				return category;
			} else
	 ...
	 *             there is no subcategory with given name
	 */
	public static MBCategory getSubcategory(long categoryId, String type)
			throws SystemException, PortalException {
	 ...
	 */
	public static void validateMessageType(long parentMessageId, int messageType)
			throws MessageTypeException, SystemException {
		...
		if (parentMessageId <= 0) {
			...
			if (!isTypeAllowed) {
				throw new MessageTypeException("Inappropriate message type: "
						+ messageType);
			}
		}
	 ...
	 *             passed up in case of an error in portal framework
	 */
	public static List<MBMessage> getChildMessages(MBMessage message)
			throws PortalException, SystemException {
		MBTreeWalker treeWalker = MBMessageLocalServiceUtil.getMessageDisplay(
				message.getMessageId(), MBThreadImpl.THREAD_VIEW_TREE)
				.getTreeWalker();
	 ...
	 *             in case of any error in portal framework
	 */
	public static MBMessage getDebateMessage(long categoryId)
			throws SystemException {
		return MBMessageLocalServiceUtil.getCategoryMessages(categoryId, 0, 1)
				.get(0);
	}
	...
	public static String getViewType(PortletRequest request) throws Exception {
		...
		long baseCategoryId = ParamUtil.getLong(request,
				DebatesConstants.BASE_CATEGORY_ID, 0);
		...
		if (baseCategoryId != 0
				&& baseCategoryId != mainCategory.getCategoryId()) {
	 ...
	 *             passed up in case of an error in portal framework
	 */
	public static long getBaseCategoryId(PortletRequest request)
			throws SystemException, PortalException {
		long categoryId = ParamUtil.getLong(request,
				DebatesConstants.CATEGORY_ID);
		long messageId = ParamUtil
				.getLong(request, DebatesConstants.MESSAGE_ID);
		...
		if (messageId > 0) {
			MBMessage message = MBMessageLocalServiceUtil
	 ...
	 *             passed up in case of framework error
	 */
	public static List<MBCategory> getCategoryCandidates(PortletRequest request)
			throws Exception {
	 ...
	 *             passed up in case of framework error
	 */
	public static List<MBCategory> getTopics(PortletRequest request)
			throws Exception {
		// topics are child categories in debate category
		...
		List<MBCategory> categories = MBCategoryLocalServiceUtil.getCategories(
				topicsCategory.getGroupId(), topicsCategory.getCategoryId());

		_log.info("Got model topics " + categories);
		return categories;
	}
	...
	public static int getTopicsCount(PortletRequest request) throws Exception {
		...
		MBCategory topicsCategory = DebatesUtil.getSubcategory(mainCategory,
				DebatesConstants.ISSUE_DEBATE_CATEGORY_NAME);
		int categoriesCount = MBCategoryLocalServiceUtil.getCategoriesCount(
				topicsCategory.getGroupId(), topicsCategory.getCategoryId());
	 ...
	 *             passed up in case of an error in portal framework
	 */
	public static List<MBCategory> getIssues(PortletRequest request,
			long topicId, int start, int end) throws SystemException {
		ThemeDisplay themeDisplay = (ThemeDisplay) request
				.getAttribute(WebKeys.THEME_DISPLAY);
		long groupId = themeDisplay.getScopeGroupId();

		List<MBCategory> issues = MBCategoryLocalServiceUtil.getCategories(
				groupId, topicId, start, end);
		return issues;
	}
	 ...
	 *             passed up in case of an error in portal framework
	 */
	public static int getIssuesCount(PortletRequest request, long topicId)
			throws SystemException {
		ThemeDisplay themeDisplay = (ThemeDisplay) request
				.getAttribute(WebKeys.THEME_DISPLAY);
		long groupId = themeDisplay.getScopeGroupId();
		int topicsCount = MBCategoryLocalServiceUtil.getCategoriesCount(
				groupId, topicId);
	...
	public static MBCategory createMBCategory(PortletRequest request,
		...
		return MBCategoryServiceUtil.addCategory(parentCategoryId, name, "",
				null, null, null, 0, false, null, null, 0, null, false, null,
				0, false, null, null, false, categoryServiceContext);
	 ...
	 *             passed up in case of an error in portal framework
	 */
	public static Map<String, Object> getDebateStatistics(MBCategory debate)
			throws SystemException, PortalException {
		MBCategory debateCategory = getSubcategory(debate.getCategoryId(),
				DebatesConstants.ISSUE_DEBATE_CATEGORY_NAME);
		...
		statistics.put(DebatesConstants.DEBATE_POSTS, debateCategory
				.getMessageCount() - 1);
		statistics.put(DebatesConstants.DISCUSSION_POSTS, discussionCategory
				.getMessageCount());
		...
		statistics.put(DebatesConstants.DEBATE_LAST_POST_DATE, debateCategory
				.getLastPostDate());

		// add discussion category id for linking
		statistics.put(DebatesConstants.DISCUSSION_CATEGORY, discussionCategory
				.getCategoryId());

		// add id of last
		MBThread thread = MBThreadLocalServiceUtil.getThreads(
				debateCategory.getCategoryId(), 0, 1).get(0);
		int debateMessagesCount = MBMessageLocalServiceUtil
				.getThreadMessagesCount(thread.getThreadId());

		// get last debate message
		MBMessage lastMessage = MBMessageLocalServiceUtil.getThreadMessages(
				thread.getThreadId(), debateMessagesCount - 1,
				debateMessagesCount).get(0);
	 ...
	 *            portlet request
	 * @param parentCategoryId
	 *            id of category that should be used as a parent for debates
	 *            topics
	 *
	 * @throws Exception
	 *             passed up in case of framework error
	 */
	public static void initializePortlet(PortletRequest request,
			long parentCategoryId) throws Exception {
		PortletPreferences preferences = request.getPreferences();

		preferences.setValue(DebatesConstants.CATEGORY_ID, Long
				.toString(parentCategoryId));
		...
		if (discussion == null) {
			discussion = DebatesUtil.createMBCategory(request,
					parentCategoryId,
					DebatesConstants.ISSUE_DISCUSSION_CATEGORY_NAME);
		}
		if (debate == null) {
		...
		preferences.setValue(DebatesConstants.DEBATE_INDEX_CATEGORY_ID, Long
				.toString(debate.getCategoryId()));
		preferences.setValue(DebatesConstants.DEBATE_DISCUSSION_CATEGORY_ID,
				Long.toString(discussion.getCategoryId()));
	 ...
	 *             passed up in case of framework error
	 */
	public static void deletePositionReferences(long messageId)
			throws PortalException, SystemException {
		List<PlanPosition> planPositions = PlanLocalServiceHelper
				.getPlanPositionsByPositionId(messageId);
