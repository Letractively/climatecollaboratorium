...
public class PlanLocalServiceHelper {
    ...
    public static Plan initializePlan(Plan plan, ActionRequest actionRequest) throws PortalException, SystemException {
        Date now = new Date();
        ...
        ServiceContext categoryServiceContext = ServiceContextFactory.getInstance(MBCategory.class.getName(),
                actionRequest);

        ServiceContext groupServiceContext = ServiceContextFactory.getInstance(Group.class.getName(),
                actionRequest);
        ...
        // populate plan with owning user and id of created group, category
        plan.setMBCategoryId(category.getCategoryId());

        long userId = themeDisplay.getUserId();
        User user = UserLocalServiceUtil.getUser(userId);

        plan.setUserId(userId);
        plan.setUserName(user.getFullName());
        plan.setUserScreenName(user.getScreenName());

        plan.setGroupId(themeDisplay.getScopeGroupId());
        plan.setCompanyId(themeDisplay.getCompanyId());

        plan.setTopicId(getDefaultTopicId(actionRequest));
        plan.setVotes(0);

        // child group represents plan community group
        plan.setChildGroupId(group.getGroupId());
    ...
    public static List<Plan> getPlans(PortletRequest request, long planTypeId, int start, int end,
        ...
        Map<Long, MBMessage> positionsMap = getPositionsMap(request, Long.parseLong(getDefaultTopicId(request)));
...
//                userSettings.
     ...
     * @throws SystemException passed up in case of framework error
     */
    public static void saveUserSettings(PortletRequest request, PlansUserSettings plansUserSettings)
            throws PortalException, SystemException {
        ...
        request.getPortletSession().setAttribute(settingsParamName, plansUserSettings);
    }
...
*/
    ...
    public static String getPlanURL(Plan p) {
        return PlanConstants.PLAN_URL.replace(PlanConstants.PLAN_REPLACEMENT_STRING, "" + p.getPlanId());
    }
