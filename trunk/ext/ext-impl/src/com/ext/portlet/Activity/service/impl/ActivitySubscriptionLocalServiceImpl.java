...
public class ActivitySubscriptionLocalServiceImpl
	...
	public boolean isSubscribed(Long userId, Long classNameId, Long classPK, int type, String extraData) throws PortalException, SystemException {
	    return activitySubscriptionPersistence.findByClassNameIdClassPKTypeExtraDataReceiverId(classNameId, classPK, type, extraData, userId).size() > 0;
	}
	
	public void addSubscription(Long classNameId, Long classPK, Integer type, String extraData, Long userId) throws PortalException, SystemException {
	    
	    if (activitySubscriptionPersistence.findByClassNameIdClassPKTypeExtraDataReceiverId(classNameId, classPK, type, extraData, userId).size() > 0) {
	   ...
	   public void addSubscription(Class clasz, Long classPK, Integer type, String extraData, Long userId) throws PortalException, SystemException {
	       ...
	       DynamicQuery query = DynamicQueryFactoryUtil.forClass(SocialActivity.class);
	        ...
	        for (ActivitySubscription sub : subscriptions) {
	            ...
	            if (sub.getExtraData() != null && sub.getExtraData().length() > 0) {
	                subCriterion = RestrictionsFactoryUtil.and(subCriterion, RestrictionsFactoryUtil.ilike("extraData", sub.getExtraData() + "%"));
	            }
