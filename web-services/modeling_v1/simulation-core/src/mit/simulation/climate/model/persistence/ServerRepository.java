...
import mit.simulation.climate.model.MetaData.Type;
...
public class ServerRepository {
	...
	public static ServerRepository instance() {
		...
		return instance;
//		DataContext c = getContextCandidate();
//		ServerRepository result = existing_repositories.get(c);
//		if (result == null) {
//			result = new ServerRepository(c);
//			existing_repositories.put(c, result);
//			logger.debug("Creating new repository "+result);
//			logger.debug("Repositories are "+existing_repositories);
//		} else {
//			logger.debug("Returning existing repository "+result);
//		}
//		return result;
	}
	...
	public void discard() {
		commit();
//		logger.debug("Deleting repository "+this);
//		logger.debug("Repositories are "+existing_repositories);
//		existing_repositories.remove(this.context);
	}
	...
	public ServerUser get(UserDAO dao) {
		if (dao==null) return null;
		ServerUser result = (ServerUser)cache.get(dao);
		if (result == null) {
			result = (ServerUser) factory.get(dao);
			putCache(dao,result);
	...
	public ServerUser findUserByName(String name) {
		SelectQuery query = new SelectQuery(UserDAO.class,ExpressionFactory.matchExp("username", name));
		List result = context.performQuery(query);
		if (result != null && result.size() > 0) {
			return get((UserDAO)result.get(0));
		}
		return null;
	}
...
//		User user = ServerRepository.instance().findUserByName("jintrone");
	...
	public static void buildATLANTA(String name) throws IOException {
		...
		if (scenario == null) {
			scenario = new ServerScenario(sim,user,"Atalanta Plan","Commitments resulting from the Atlanta Negotiation Excercise");
