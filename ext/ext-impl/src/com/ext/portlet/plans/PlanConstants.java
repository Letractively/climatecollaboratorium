...
//					(plan.getVotes()+" ("+String.valueOf(100*plan.getVotes() / votes)+"%)");
		...
		CREATOR("Creator","Creator of this plan","ShowCreator",false, Attribute.CREATOR,new PlanValueFactory() {
			...
			public String getValue(Plan plan) throws SystemException {
				try {
				return CommunityUtil.generateUserURL(plan.getUserId());
		...
		public void setUserSetting(PlansUserSettings settings,boolean b) throws PortalException, SystemException {
			try {
			    settings.getColumnSettings(String.valueOf(this)).setVisible(b);
			    settings.addColumnSettings(settings.getColumnSettings(String.valueOf(this)));
			    
				//Method m = PlansUserSettings.class.getMethod(setter, Boolean.class);
				//m.invoke(settings, b);
            } 
			catch (NoSuchPlanColumnSettingsException e) {
                PlanLocalServiceHelper.createColumnSettings(settings, this).setVisible(b);
            }
			catch (Exception e) {
		...
		public boolean getUserSetting(PlansUserSettings settings) throws PortalException, SystemException {
			try {
			    if (settings.getColumnSettings(String.valueOf(this)) == null) {
			        return PlanLocalServiceHelper.createColumnSettings(settings, this).getVisible();
			    }
			    return settings.getColumnSettings(String.valueOf(this)).getVisible();
			} catch (NoSuchPlanColumnSettingsException e) {
			    return PlanLocalServiceHelper.createColumnSettings(settings, this).getVisible();
   ...
   /**
	...
	public static final int STATUS_APPROVED = MembershipRequestImpl.STATUS_APPROVED;
	
	public static final int STATUS_PENDING = MembershipRequestImpl.STATUS_PENDING;
	
	public static final int STATUS_DENIED = MembershipRequestImpl.STATUS_DENIED;
