...
public class HelpMessageTriggerRenderer extends DomBasicRenderer {
    ...
    /**
     * Whether or not the message should be open by default
     */
    ...
    public void encodeBegin(FacesContext facesContext, UIComponent uiComponent) throws IOException {
        ...
        if (hmt.getStyleClass() != null && !hmt.getStyleClass().trim().equals("")) {
         ...
         HelpMessage msg = getMessage(facesContext, hmt);
        if (isHelpVisible(facesContext,defaultOpen(msg))) {
     ...
     * @return
     */
    private boolean isHelpVisible(FacesContext facesContext, boolean defaultOpen) {
    ...
    public void decode(FacesContext facesContext, UIComponent uiComponent) {
       ...
       if (uiComponent instanceof HelpMessageTrigger) {
            HelpMessageTrigger trigger = ((HelpMessageTrigger) uiComponent);

            ThemeDisplay td = Helper.getThemeDisplay();
            PortletSession session = (PortletSession) facesContext.getExternalContext().getSession(true);
            messageId = ((HelpMessageTrigger) uiComponent).getMessageId();
            HelpMessage msg = getMessage(facesContext, trigger);
            ...
            if (facesContext.getExternalContext().getRequestParameterMap().get(MESSAGE_TRIGGER + uiComponent.getClientId(facesContext)).toString().trim().equals("true")) {
                try {
                    HelpUserSettingHelper.toogleHelpUserSetting(td.getUserId(), messageId, session, defaultOpen(msg));
                } catch (PortalException e) {
                    e.printStackTrace();
                } catch (SystemException e) {
                    e.printStackTrace();
    ...
    public static boolean defaultOpen(HelpMessage msg) {
        return "open".equals(msg.getDefaultState());
    }


    public HelpMessage getMessage(FacesContext facesContext, HelpMessageTrigger trigger) {
        HelpMessage msg = trigger.getMessageComponent();

        if (msg == null) {
         msg = (HelpMessage) findHelpMessageTag(facesContext.getViewRoot(), messageId);
            if (msg == null)
                throw new FacesException("Cannot identify related help message tag for trigger " + messageId);
            trigger.setMessageComponent(msg);
    ...
    public UIComponent findHelpMessageTag(UIComponent comp, String messageId) {
        if (comp instanceof HelpMessage){
            if (messageId.equals(((HelpMessage) comp).getMessageId())) {
                return comp;
            } else {

            }
        } else {
            for (Iterator i =comp.getFacetsAndChildren();i.hasNext();) {
                
                UIComponent result = findHelpMessageTag((UIComponent) i.next(), messageId);
